...
# -------------------------------------------------------------------
# WEBHOOK 3 — EXCLUSÃO OEE / LINHA 01
# -------------------------------------------------------------------

@server.post("/lark/excluir-oee-linha01")
def excluir_oee():
    if not validar_token():
        return jsonify({"status": "erro", "mensagem": "Token incorreto"}), 401
    ...
    with conectar() as con:
        cur = con.execute("DELETE FROM oee_linha01 WHERE id = ?", (rid,))
...
# -------------------------------------------------------------------
# WEBHOOK 4 — EXCLUSÃO PROGRAMAÇÃO / LINHA 01
# -------------------------------------------------------------------

@server.post("/lark/excluir-programacao-linha01")
def excluir_programacao():
    if not validar_token():
        return jsonify({"status": "erro", "mensagem": "Token incorreto"}), 401
    ...
    with conectar() as con:
        cur = con.execute("DELETE FROM programacao_linha01 WHERE id = ?", (rid,))
        removidos = cur.rowcount

    return jsonify({
        "status": "ok",
        "tipo": "exclusao_programacao",
        "id": rid,
        "removidos": removidos
    })
...
# -------------------------------------------------------------------
# WEBHOOK — LIMPAR TODOS OS DADOS DA LINHA 01
# Use apenas de forma controlada. Exige o mesmo X-Webhook-Token.
# -------------------------------------------------------------------

@server.post("/lark/limpar-linha01")
def limpar_linha01():
    if not validar_token():
        return jsonify({"status": "erro", "mensagem": "Token incorreto"}), 401

    with conectar() as con:
        oee = con.execute("DELETE FROM oee_linha01").rowcount
        prog = con.execute("DELETE FROM programacao_linha01").rowcount

    return jsonify({
        "status": "ok",
        "tipo": "limpeza_total_linha01",
        "oee_removidos": oee,
        "programacao_removidos": prog
    })
...
def api_linha01():
    ...
    if not oee:
        return jsonify({
            ...
            "programacaoAviso": "Nenhum registro OEE da Linha 01 no servidor."
        })
